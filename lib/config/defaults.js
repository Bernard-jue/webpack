...
const applyWebpackOptionsDefaults = options => {
	...
	F(options, "performance", () =>
		production && targetProperties && !targetProperties.downloadSizeIrrelevant
			? {}
			: false
	);
...
const applyModuleDefaults = (
	module,
	{ cache, mjs, syncWebAssembly, asyncWebAssembly }
) => {
...
const applyOutputDefaults = (
	output,
	{ context, targetProperties, outputModule, development, entry }
) => {
	/**
	...
	F(output, "globalObject", () => {
		if (targetProperties) {
			if (targetProperties.global) return "global";
			if (targetProperties.globalThis) return "globalThis";
		}
		return "self";
	});
	F(output, "chunkFormat", () => {
		if (targetProperties) {
			if (targetProperties.require) return "commonjs";
			if (targetProperties.node) return "commonjs";
			if (targetProperties.document) return "array-push";
			if (targetProperties.importScripts) return "array-push";
		}
		return false;
	});
	F(output, "chunkLoading", () => {
		if (targetProperties) {
			if (targetProperties.require) return "commonjs";
			if (targetProperties.node) return "async-node";
			if (targetProperties.document) return "jsonp";
			if (targetProperties.importScripts) return "import-scripts";
		}
		return false;
	});
	F(output, "workerChunkLoading", () => {
		if (targetProperties) {
			...
			if (targetProperties.node) return "async-node";
			if (targetProperties.importScripts) return "import-scripts";
			if (targetProperties.importScriptsInWorker) return "import-scripts";
		}
		return false;
	});
	F(output, "wasmLoading", () => {
		if (targetProperties) {
			if (targetProperties.fetchWasm) return "fetch";
			if (targetProperties.node) return "async-node";
		}
		return false;
	});
	...
	D(output.environment, "destructuring", true);
...
const applyExternalsPresetsDefaults = (
	...
	D(
		externalsPresets,
		"electronMain",
		targetProperties && targetProperties.electronMain
	);
	D(
		externalsPresets,
		"electronPreload",
		targetProperties && targetProperties.electronPreload
	);
};
